  ...
  return (
    <div className="min-h-screen bg-gray-50 flex items-center justify-center p-6">
        ...
        <form className="grid gap-3" onSubmit={submit}>
          ...
          <input className="w-full border rounded px-3 py-2" placeholder="Nova senha" type="password" value={password} onChange={e => setPassword(e.target.value)} />
          {error && <div className="text-red-600 text-sm">{error}</div>}
          {ok && <div className="text-green-700 text-sm">Senha alterada. Faça login.</div>}
          <button className="w-full sm:w-auto bg-blue-600 text-white rounded px-4 py-2">Confirmar</button>
        </form>
      </div>
    </div>
  )
