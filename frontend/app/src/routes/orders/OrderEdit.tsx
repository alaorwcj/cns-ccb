...
export default function OrderEdit() {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        setOrder(ord.data)
        setChurchId(ord.data.church_id)
        const initialItems: Record<number, number> = {}
        ord.data.items.forEach((it: any) => {
  ...
  const submit = async () => {
    ...
    try {
      ...
      await api.put(`/orders/${id}`, { church_id: churchId, items: chosen })
      alert('Pedido atualizado')
      navigate('/orders')
    } catch (e: any) {
      setError(e?.response?.data?.detail || e?.message || 'Falha ao atualizar pedido')
    }
  }
  ...
  return (
    <div className="grid gap-4">
      ...
      <div className="flex gap-2 items-center">
        ...
        <button className="bg-blue-600 text-white rounded px-3 py-1" onClick={submit}>Atualizar Pedido</button>
      </div>
